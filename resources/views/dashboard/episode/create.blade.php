...
<main>
  ...
  <div title="New Episode" class=" max-w-screen-md flex items-center realative p-1 m-auto">
  ...
  <div class="relative max-w-screen-md grid m-auto">
    <div class="absolute w-full">
      <div class="mt-3 absolute bg-dashform w-full md:h-screen p-5">
        <form action="/dashboard/episode" method="POST" enctype="multipart/form-data">
          @csrf
          <input type="text" name="episode_id" value="{{ $latestIdUsingOrderBy  }}">
          <div class="mb-5">
          ...
          <div class="mb-5">
            <label for="slug">Slug</label> <br>
              <input type="text" name="slug" id="slug" class="form-control block w-full px-4 py-1 text-lg font-normal text-gray-700 bg-slate-200 bg-clip-padding border-2 border-b-slate-400 transition ease-in-out m-0 focus:text-gray-700 focus:bg-slate-100  focus:outline-none placeholder:text-slate-500" value="{{ old('slug') }}" placeholder="slug...">
              @error('slug')
              {{ $message }}
          @enderror
          </div>
          <div class="mb-3">
            <input type="text" id="srcSlc" class="form-control block w-[30%] px-4 py-1 text-sm font-normal text-gray-700 bg-slate-200 bg-clip-padding border-2 border-b-slate-400 transition ease-in-out m-0 focus:text-gray-700 focus:bg-slate-100  focus:outline-none placeholder:text-slate-500" placeholder="Search Series..">
          </div>
          <div class="mb-5 flex">
            <select name="series_id" id="select">
...
button.addEventListener("click", function(event) {
  ...
  document.getElementById("dup").appendChild(img);
    ...
    var input = document.getElementById('srcSlc');
